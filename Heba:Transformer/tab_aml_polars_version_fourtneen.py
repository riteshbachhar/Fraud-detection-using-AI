...
# =============================================================================
# Example main function to run everything
# =============================================================================

def main(dataset_path: str, label_col: str, cat_cols: List[str], cont_cols: List[str]):
    # Load CSV via Polars
    df = pl.read_csv(dataset_path)

    # Preprocess, recast, split
    df = preprocess_saml_d(df)
    df = recast(df)
    train_df, val_df, test_df = custom_split_polars(df)

    # Create datasets
    train_ds = TabAMLDataset(train_df, cat_cols, cont_cols, label_col, fit=True)
    val_ds = TabAMLDataset(val_df, cat_cols, cont_cols, label_col,
                           encoders=train_ds.encoders, scaler=train_ds.scaler, fit=False)
    test_ds = TabAMLDataset(test_df, cat_cols, cont_cols, label_col,
                            encoders=train_ds.encoders, scaler=train_ds.scaler, fit=False)

    # Weighted sampler to handle class imbalance
    train_sampler = build_weighted_sampler(pl.Series(train_df[label_col]))

    # DataLoaders
    train_loader = DataLoader(train_ds, batch_size=256, sampler=train_sampler, collate_fn=lambda b: b[0])
    val_loader = DataLoader(val_ds, batch_size=512, shuffle=False, collate_fn=lambda b: b[0])
    test_loader = DataLoader(test_ds, batch_size=512, shuffle=False, collate_fn=lambda b: b[0])

    # Model instantiation
    model = TabAMLModel(
        category_sizes=train_ds.category_sizes,
    ...
    pos_weight = compute_pos_weight(pl.Series(train_df[label_col]))
    loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_weight], device=device))
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3)

    # Training loop (e.g., 3 epochs)
    epochs = 3
    for epoch in range(epochs):
        train_loss = train_epoch(model, train_loader, optimizer, loss_fn, device)
        val_metrics = evaluate(model, val_loader, loss_fn, device)
        print(f"Epoch {epoch+1}/{epochs} | Train loss: {train_loss:.4f} | "
              f"Val loss: {val_metrics[0]:.4f} | AUC-ROC: {val_metrics[1]:.4f} | AUC-PR: {val_metrics[2]:.4f}")

    # Final evaluation on test
    test_metrics = evaluate(model, test_loader, loss_fn, device)
    print(f"Test Metrics: Loss={test_metrics[0]:.4f}, AUC-ROC={test_metrics[1]:.4f}, "
          f"AUC-PR={test_metrics[2]:.4f}, Recall={test_metrics[3]:.4f}, "
          f"Precision={test_metrics[4]:.4f}, F1={test_metrics[5]:.4f}, Confusion={test_metrics[6]}")
