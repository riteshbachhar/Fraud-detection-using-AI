...
def train_epoch(model, loader, optimizer, loss_fn, device,
                warmup_steps, base_lr, global_step, max_norm=2.0):

    model.train()
    total, n = 0.0, 0

    for batch in loader:
        global_step += 1  # track progress for warm-up
        ...
        #Print LR every 50 steps
        if global_step % 50 == 0:
            print(f"[Warmup] Step {global_step} | LR: {optimizer.param_groups[0]['lr']:.6f}")
